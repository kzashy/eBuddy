...
namespace Dancing_Cassio
{
    internal class Cassiopeia
    {
        ...
        private static void Harass()
        {
            ...
            {
                foreach (
                    var target in EntityManager.Heroes.Enemies.Where(o => o.IsValidTarget(Q.Range) && !o.IsDead && !o.IsZombie))
                {
            ...
            {
                foreach (
                    var target in EntityManager.Heroes.Enemies.Where(o => o.IsValidTarget(E.Range) && !o.IsDead && !o.IsZombie
        ...
        private static void LaneClear()
        {
            ...
            var minionsQ = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, Player.Instance.Position, Q.Range).Where(minion => minion != null && !minion.IsDead).ToList();
            var minionsW = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, Player.Instance.Position, W.Range).Where(minion => minion != null && !minion.IsDead).ToList();
            var minionsE = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy, Player.Instance.Position, E.Range).Where(minion => minion != null && !minion.IsDead).ToList();
            if (!minionsQ.Any()) return;
            if (useQ && Q.IsReady())
            {
                var pred = Prediction.Position.PredictCircularMissileAoe(minionsQ.ToArray(), 750,
                    40, 750, 0);
                if (pred.Any())
                {
            ...
            if (useW && W.IsReady())
            {
                var pred = Prediction.Position.PredictCircularMissileAoe(minionsW.ToArray(), 850, 500, 0, 90);
                if (pred.Any())
                {
                    var pred2 = pred.OrderByDescending(a => a.CollisionObjects.Length).FirstOrDefault();
